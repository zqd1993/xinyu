...
object DateUtils {
    ...
    fun timedate(time: String, type: String?): String? {
        val sdr = SimpleDateFormat(type)
    ...
    fun getFirstDayOfThisMonth(): String? {
        return getFirstDayOfMonth(getDateFormatString(System.currentTimeMillis(), "yyyy-MM-dd"))
    }

    fun getFirstDayOfMonth(ym: String): String? {
        val arr = ym.split("-").toTypedArray()
    ...
    fun getLastDayOfMonth(ym: String): String? {
        val arr = ym.split("-").toTypedArray()
        val year = arr[0].toInt()
        val month = arr[1].toInt()
        val cal = Calendar.getInstance()
