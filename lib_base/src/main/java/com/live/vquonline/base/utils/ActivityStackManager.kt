...
object ActivityStackManager {
    ...
    val activitySingleInstanceStack by lazy { Stack<Activity>() }
    ...
    fun isHasActivity(activityClass: Class<*>): Boolean {
        activityStack.toList().reversed().forEach {
            if (it.javaClass == activityClass) {
                return true
            }
        }
    ...
    fun addSingleInstanceActivityToStack(activity: Activity) {
        activityStack.push(activity)
    }

    fun removeSingleInstanceActivityToStack(activity: Activity) {
        activityStack.remove(activity)
    }

    fun isHasSingleInstanceActivity(activityClass: Class<*>): Boolean {
        activitySingleInstanceStack.toList().reversed().forEach {
            if (it.javaClass == activityClass) {
                return true
            }
        }
