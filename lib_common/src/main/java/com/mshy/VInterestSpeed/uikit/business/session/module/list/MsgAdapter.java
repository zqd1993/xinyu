...
public class MsgAdapter extends BaseMultiItemFetchLoadAdapter<IMMessage, BaseViewHolder> {
    ...
    MsgAdapter(RecyclerView recyclerView, List<IMMessage> data, Container container) {
        ...
        for (Class<? extends MsgViewHolderBase> holder : holders) {
            viewType++;
            addItemType(viewType, R.layout.common_tanta_nim_message_item, holder);
...
//            notifyDataSetChanged(); // 可以不要！！！
    ...
    public void fetchMoreEnd(List<IMMessage> data, boolean gone) {
        ...
        if (gone && !getContainer().account.equals("4")) {
            getContainer().proxy.getChatInfo();
        }
    }

    public void setInfoIMMessage(JSONObject data) {
        //为ture表示没有其他数据了 可以显示资料卡
        if (mData.isEmpty() || !(mData.get(0).getAttachment() instanceof MessageVquMsgInfoAttachment)) {
            MessageVquMsgInfoAttachment attachment = new MessageVquMsgInfoAttachment();
            attachment.setJsonData(data);
            IMMessage stickerMessage = MessageBuilder.createCustomMessage(container.account, container.sessionType,
                    attachment);
            stickerMessage.setStatus(MsgStatusEnum.success);
            add(0, stickerMessage);
        }
    }
