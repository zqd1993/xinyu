...
import com.mshy.VInterestSpeed.uikit.common.media.audioplayer.Playable;
...
public class MsgViewHolderAudio extends MsgViewHolderBase {
    ...
    protected void onItemClick() {
        ...
        if (userInCall != null && userInCall.isUserInCall()) {
            String typeAudio = userInCall.isAudio() ? "语音" : "视频";
            ToastHelper.showToast(context, "正在" + typeAudio + "通话中，请稍后再试...");
            return;
        }
    ...
    private void layoutByDirection() {
        if (isReceivedMessage()) {
            ...
            animationView.setBackgroundResource(R.drawable.resources_vqu_nim_audio_animation_list_left);
            ...
            animationView.setBackgroundResource(R.drawable.resources_vqu_nim_audio_animation_list_right);
    ...
    private MessageAudioControl.AudioControlListener onPlayListener = new MessageAudioControl.AudioControlListener() {
        ...
        public void onEndPlay(Playable playable) {
            ...
            if (getMsgAdapter() != null) {
                getMsgAdapter().getContainer().activity.runOnUiThread(new Runnable() {
                    @Override
                    public void run() {
    ...
    private void endPlayAnim() {
        if (isReceivedMessage()) {
            animationView.setBackgroundResource(R.mipmap.resources_vqu_nim_audio_animation_list_left_3);
        } else {
            animationView.setBackgroundResource(R.mipmap.resources_vqu_nim_audio_animation_list_right_3);
        }
    }
