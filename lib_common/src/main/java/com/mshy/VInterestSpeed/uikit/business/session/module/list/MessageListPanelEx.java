package com.mshy.VInterestSpeed.uikit.business.session.module.list;
...
import com.mshy.VInterestSpeed.uikit.business.robot.parser.elements.group.LinkElement;
import com.mshy.VInterestSpeed.uikit.business.session.activity.VoiceTrans;
...
import com.mshy.VInterestSpeed.uikit.business.session.helper.MessageListPanelHelper;
import com.mshy.VInterestSpeed.uikit.business.session.module.Container;
import com.mshy.VInterestSpeed.uikit.business.session.viewholder.robot.RobotLinkView;
import com.mshy.VInterestSpeed.uikit.common.CommonUtil;
import com.mshy.VInterestSpeed.uikit.common.ToastHelper;
import com.mshy.VInterestSpeed.uikit.common.ui.dialog.CustomAlertDialog;
import com.mshy.VInterestSpeed.uikit.common.ui.dialog.EasyAlertDialog;
import com.mshy.VInterestSpeed.uikit.common.ui.dialog.EasyAlertDialogHelper;
import com.mshy.VInterestSpeed.uikit.common.ui.recyclerview.adapter.BaseFetchLoadAdapter;
import com.mshy.VInterestSpeed.uikit.common.ui.recyclerview.adapter.IRecyclerView;
import com.mshy.VInterestSpeed.uikit.common.ui.recyclerview.listener.OnItemClickListener;
import com.mshy.VInterestSpeed.uikit.common.ui.recyclerview.loadmore.MsgListFetchLoadMoreView;
import com.mshy.VInterestSpeed.uikit.common.util.media.BitmapDecoder;
import com.mshy.VInterestSpeed.uikit.common.util.sys.ClipboardUtil;
import com.mshy.VInterestSpeed.uikit.common.util.sys.NetworkUtil;
...
//        if (SPUtils.getInt(BaseApplication.getInstance(), "openGreen") == 1) {
//            IMMessage imMessage = MessageBuilder.createTipMessage(container.account, container.sessionType);
//            imMessage.setStatus(MsgStatusEnum.success);
//            imMessage.setContent("在聊天过程中，涉及合同汇款、转账 、资金、中奖、回款等信息，请核实对方身份，以避免被骗导致钱款损失。");
//            items.add(0, imMessage);
//            adapter.notifyDataSetChanged();
//        }
    }

    public void setAdapterInfo(JSONObject data) {
        if (adapter != null) {
            adapter.setInfoIMMessage(data);
        }
    }
    ...
    public void onIncomingMessage(List<IMMessage> messages) {
        ...
        for (IMMessage message : messages) {
            if (isMyMessage(message)) {
                items.add(message);
                LogUtils.e("onIncomingMessage" + GsonUtil.GsonString(messages));
...
//                incomingMsgPrompt.show(lastMsg);
    ...
    private void sortMessages(List<IMMessage> list) {
        ...
        for (int i = 0; i < list.size(); i++) {//把资料卡消息放在最上面
            if (list.get(i).getAttachment() instanceof MessageVquMsgInfoAttachment) {
                Log.e("TAG", "sortMessages: " + i);
...
//            MessageHelper.getInstance().onRevokeMessage(message,message.getSessionId());//撤回消息了 插入一条提醒消息
        }
    };
    ...
    private class MsgItemEventListener implements MsgAdapter.ViewHolderEventListener {
        ...
        public void onAudioClick() {
            //音频
            container.proxy.callAudio(container.account);
        }

        @Override
        public void onVideoClick() {
            //视频
            container.proxy.callVideo(container.account);
        }
...
//            }
    ...
    public void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (requestCode == REQUEST_CODE_FORWARD_TEAM || requestCode == REQUEST_CODE_FORWARD_PERSON) {
