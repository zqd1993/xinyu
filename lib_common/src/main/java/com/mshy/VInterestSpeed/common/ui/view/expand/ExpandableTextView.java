package com.mshy.VInterestSpeed.common.ui.view.expand;
...
//                    }
    ...
    private void open() {
        if (hasAnimation) {
            ...
            mOpenHeight = layout.getHeight() + getPaddingTop() + getPaddingBottom()+ UiUtils.dip2px(BaseApplication.context, 80);
            executeOpenAnim();
        } else {
    ...
    private void executeCloseAnim() {
        //创建收起动画
        if (mCloseAnim == null) {
            mCloseAnim = new ExpandCollapseAnimation(this, mOpenHeight, mCLoseHeight+UiUtils.dip2px(BaseApplication.context, 50));
