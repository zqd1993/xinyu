 ...
 * description:
 */
class CommonTantaRechargeAdapter :
    BaseQuickAdapter<CommonTantaRechargeOptions, BaseViewHolder>(R.layout.common_tanta_item_recharge_options) {
...
//        tvDiamond.isSelected = item.isSelected
        ...
        if (!item.btnText.isNullOrEmpty()) {
            tvSale.visibility = View.VISIBLE
            val saleText = StringBuilder()
            item.btnText.forEachIndexed { index, s ->
                if (index < item.btnText.size - 1) {
