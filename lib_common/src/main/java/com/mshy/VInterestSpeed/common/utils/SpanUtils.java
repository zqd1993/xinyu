...
public final class SpanUtils {
    ...
    public SpanUtils setClickSpan(@NonNull final ClickableSpan clickSpan) {
        if (mTextView != null && mTextView.getMovementMethod() == null) {
            mTextView.setMovementMethod(LinkMovementClickMethod.getInstance());
        }
...
//            LogUtils.e(fm, sfm);
    ...
    static class CustomImageSpan extends CustomDynamicDrawableSpan {
        ...
        private CustomImageSpan(final Bitmap b, final int verticalAlignment) {
            ...
            mDrawable = new BitmapDrawable(sContext.getResources(), b);
            mDrawable.setBounds(
        ...
        public Drawable getDrawable() {
            ...
            if (mDrawable != null) {
                ...
                try {
                    InputStream is =
                            sContext.getContentResolver().openInputStream(mContentUri);
                    bitmap = BitmapFactory.decodeStream(is);
                    drawable = new BitmapDrawable(sContext.getResources(), bitmap);
                    drawable.setBounds(
                ...
                try {
                    drawable = ContextCompat.getDrawable(sContext, mResourceId);
                    drawable.setBounds(
                            0, 0, drawable.getIntrinsicWidth(), drawable.getIntrinsicHeight()
                    );
                } catch (Exception e) {
